...
function loop() {
  ...
  var cleared = false;
   if(lastDayDraw != d.getDate()){
     ...
     //drawDate(d);//We don't need a full date
     drawCal(d);
   }
...
function drawTime(d){
  ...
  g.drawString(hour,60,10);
  g.drawString(min, 60,50);
  g.setColor(accentColor);
  g.drawLine(0,47,70,47);
...
function drawCal(d){
  ...
  g.setFont("Vector",10);
  g.setColor(accentColor);
  g.drawString(days[6],6*cellSize+8,calStart -11); // display the sunday label
  //Sunday is the only label needed, we know our days of week. 
  g.setColor(fontColor);
  g.setFontAlign(-1,-1,0);
  g.drawString("GMT: "+d.toISOString().slice(10,16),3,calStart-11);
  for(i = 0; i < days.length;i++){
    //g.drawString(days[i],i*cellSize+5,calStart -11);//disable day-of-week 
    if(i!=0){
